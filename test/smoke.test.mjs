...
test.skip("Pro removes the calendar cap, the window cap and the export cap", async () => {
  const s = sandbox();
...
test.skip("empty state and bad input answer with instructions, never a stack trace", async () => {
  const s = sandbox();
  const c = client(s.env);
  try {
    await c.init();
